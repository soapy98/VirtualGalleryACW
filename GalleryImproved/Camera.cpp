...
Camera::Camera() :pitch(), yaw(), projectionMatrix(),m_pos(XMFLOAT3(0.0f, 0.0f, 0.0f))
{
...
void Camera::SetLookAtPos(XMFLOAT3& lookAtPos)
{
	...
	if (CompareFloat(lookAtPos.x, this->m_pos.x) && CompareFloat(lookAtPos.y, this->m_pos.y) && CompareFloat(lookAtPos.z, this->m_pos.z))
		return;
	lookAtPos.x = this->m_pos.x - lookAtPos.x;
	lookAtPos.y = this->m_pos.y - lookAtPos.y;
	lookAtPos.z = this->m_pos.z - lookAtPos.z;
	float pitc = 0.0f;
	if (lookAtPos.y > 0.0f|| lookAtPos.y < 0.0f)
	{
		const float distance = sqrt(lookAtPos.x * lookAtPos.x + lookAtPos.z * lookAtPos.z);
		pitc = atan(lookAtPos.y / distance);
	}
	float ya = 0.0f;
	if (lookAtPos.x > 0.0f || lookAtPos.x < 0.0f)
	{
